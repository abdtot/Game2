// settings.js - إدارة شاشة الإعدادات

// متغيرات الإعدادات
let soundEnabled = true;
let vibrationEnabled = true;
let notificationsEnabled = true;
let deferredPrompt = null;

// عناصر التحكم في الصوت لكل نوع صوت
const soundControls = {
    click: { enabled: true, volume: 0.7 },
    flip: { enabled: true, volume: 0.7 },
    success: { enabled: true, volume: 0.7 },
    error: { enabled: true, volume: 0.7 },
    win: { enabled: true, volume: 0.8 },
    notification: { enabled: true, volume: 0.6 },
    levelComplete: { enabled: true, volume: 0.8 },
    star: { enabled: true, volume: 0.6 },
    background: { enabled: true, volume: 0.3 },
    culturalPuzzle: { enabled: true, volume: 0.7 },
    hint: { enabled: true, volume: 0.6 },
    unlock: { enabled: true, volume: 0.7 },
    coin: { enabled: true, volume: 0.5 },
    achievement: { enabled: true, volume: 0.7 },
    timer: { enabled: true, volume: 0.6 },
    applause: { enabled: true, volume: 0.8 }
};

// تهيئة إعدادات المستخدم
function loadUserSettings() {
    if (!db) return;
    
    const transaction = db.transaction(['settings'], 'readonly');
    const settingsStore = transaction.objectStore('settings');
    
    const vibrationRequest = settingsStore.get('vibration');
    vibrationRequest.onsuccess = (event) => {
        if (vibrationRequest.result) {
            vibrationEnabled = vibrationRequest.result.value;
            window.vibrationEnabled = vibrationEnabled;
            const toggle = document.getElementById('vibration-toggle');
            if (toggle) toggle.checked = vibrationEnabled;
        }
    };
    
    const soundRequest = settingsStore.get('sound');
    soundRequest.onsuccess = (event) => {
        if (soundRequest.result) {
            soundEnabled = soundRequest.result.value;
            window.soundEnabled = soundEnabled;
            const toggle = document.getElementById('sound-toggle');
            if (toggle) toggle.checked = soundEnabled;
            
            // تحديث نظام الصوت
            if (audioSystem) {
                audioSystem.isEnabled = soundEnabled;
            }
        }
    };
    
    const notificationsRequest = settingsStore.get('notifications');
    notificationsRequest.onsuccess = (event) => {
        if (notificationsRequest.result) {
            notificationsEnabled = notificationsRequest.result.value;
            window.notificationsEnabled = notificationsEnabled;
            const toggle = document.getElementById('notifications-toggle');
            if (toggle) toggle.checked = notificationsEnabled;
        }
    };
    
    const volumeRequest = settingsStore.get('volume');
    volumeRequest.onsuccess = (event) => {
        if (volumeRequest.result) {
            if (audioSystem) {
                audioSystem.setVolume(volumeRequest.result.value);
            }
        }
    };
    
    // تحميل إعدادات كل صوت على حدة
    loadSoundControls();
}

// تحميل إعدادات التحكم في الأصوات
function loadSoundControls() {
    if (!db) return;
    
    const transaction = db.transaction(['soundSettings'], 'readonly');
    const soundSettingsStore = transaction.objectStore('soundSettings');
    
    Object.keys(soundControls).forEach(soundName => {
        const request = soundSettingsStore.get(soundName);
        request.onsuccess = (event) => {
            if (request.result) {
                soundControls[soundName] = request.result.value;
                updateSoundControlUI(soundName, request.result.value);
            }
        };
    });
}

// تحديث واجهة التحكم في الصوت
function updateSoundControlUI(soundName, settings) {
    const soundElement = document.getElementById(`${soundName}-control`);
    if (!soundElement) return;
    
    const toggle = soundElement.querySelector('.sound-toggle');
    const volumeSlider = soundElement.querySelector('.sound-volume');
    
    if (toggle) {
        toggle.checked = settings.enabled;
    }
    
    if (volumeSlider) {
        volumeSlider.value = settings.volume * 100;
        
        // تحديث شريط التقدم للصوت
        const volumeFill = volumeSlider.nextElementSibling;
        if (volumeFill && volumeFill.classList.contains('volume-fill')) {
            volumeFill.style.width = `${settings.volume * 100}%`;
        }
        
        // تحديث قيمة النسبة المئوية
        const volumePercent = volumeSlider.parentElement.querySelector('.volume-percent');
        if (volumePercent) {
            volumePercent.textContent = `${Math.round(settings.volume * 100)}%`;
        }
    }
}

// حفظ إعدادات الصوت
function saveSoundControl(soundName, settings) {
    if (!db) return;
    
    const transaction = db.transaction(['soundSettings'], 'readwrite');
    const soundSettingsStore = transaction.objectStore('soundSettings');
    soundSettingsStore.put({ id: soundName, value: settings });
    
    // تطبيق الإعدادات على نظام الصوت الفعلي
    if (audioSystem && audioSystem.sounds[soundName]) {
        audioSystem.sounds[soundName].volume = settings.enabled ? settings.volume : 0;
    }
}

// إعداد مستمعي أحداث شاشة الإعدادات
function setupSettingsEventListeners() {
    // تبديل الصوت العام
    const soundToggle = document.getElementById('sound-toggle');
    if (soundToggle) {
        soundToggle.addEventListener('change', (e) => {
            soundEnabled = e.target.checked;
            window.soundEnabled = soundEnabled;
            saveSetting('sound', soundEnabled);
            
            // تطبيق الإعداد على نظام الصوت
            if (audioSystem) {
                audioSystem.isEnabled = soundEnabled;
                
                if (soundEnabled) {
                    audioSystem.resumeBackgroundMusic();
                    playSound('click');
                } else {
                    audioSystem.pauseBackgroundMusic();
                }
            }
        });
    };
    
    // تبديل الاهتزاز
    const vibrationToggle = document.getElementById('vibration-toggle');
    if (vibrationToggle) {
        vibrationToggle.addEventListener('change', (e) => {
            vibrationEnabled = e.target.checked;
            window.vibrationEnabled = vibrationEnabled;
            saveSetting('vibration', vibrationEnabled);
            
            if (vibrationEnabled) vibrate(50);
        });
    };
    
    // تبديل الإشعارات
    const notificationsToggle = document.getElementById('notifications-toggle');
    if (notificationsToggle) {
        notificationsToggle.addEventListener('change', (e) => {
            notificationsEnabled = e.target.checked;
            window.notificationsEnabled = notificationsEnabled;
            saveSetting('notifications', notificationsEnabled);
            
            if (notificationsEnabled) {
                showToast('تم تفعيل الإشعارات');
            }
        });
    };
    
    // ألوان السمات
    document.querySelectorAll('.color-option').forEach(option => {
        option.addEventListener('click', () => {
            handleColorChange(option);
        });
    });
    
    // إعدادات الصوت التفصيلية
    setupSoundControlsEventListeners();
    
    // مشاركة التطبيق
    const shareBtn = document.getElementById('share-app');
    if (shareBtn) {
        shareBtn.addEventListener('click', shareApp);
    }
    
    // تقييم التطبيق
    const rateBtn = document.getElementById('rate-app');
    if (rateBtn) {
        rateBtn.addEventListener('click', rateApp);
    }
    
    // إعادة ضبط التطبيق
    const resetBtn = document.getElementById('reset-app');
    if (resetBtn) {
        resetBtn.addEventListener('click', resetApp);
    }
    
    // المساعدة
    const helpBtn = document.getElementById('help-btn');
    if (helpBtn) {
        helpBtn.addEventListener('click', showHelpGuide);
    }
    
    // اتصل بنا
    const contactBtn = document.getElementById('contact-btn');
    if (contactBtn) {
        contactBtn.addEventListener('click', showContactForm);
    }
    
    // سياسة الخصوصية
    const privacyBtn = document.getElementById('privacy-btn');
    if (privacyBtn) {
        privacyBtn.addEventListener('click', showPrivacyPolicy);
    }
    
    // تثبيت التطبيق
    const installBtn = document.getElementById('install-app');
    if (installBtn) {
        installBtn.addEventListener('click', installApp);
    }
}

// إعداد مستمعي أحداث التحكم في الأصوات
function setupSoundControlsEventListeners() {
    Object.keys(soundControls).forEach(soundName => {
        const controlElement = document.getElementById(`${soundName}-control`);
        if (!controlElement) return;
        
        // مستمع لتبديل الصوت
        const toggle = controlElement.querySelector('.sound-toggle');
        if (toggle) {
            toggle.addEventListener('change', (e) => {
                const enabled = e.target.checked;
                soundControls[soundName].enabled = enabled;
                saveSoundControl(soundName, soundControls[soundName]);
                
                // تشغيل صوت تجريبي
                if (enabled && audioSystem) {
                    audioSystem.play(soundName);
                }
            });
        };
        
        // مستمع لشريط التحكم في مستوى الصوت
        const volumeSlider = controlElement.querySelector('.sound-volume');
        if (volumeSlider) {
            volumeSlider.addEventListener('input', (e) => {
                const volume = e.target.value / 100;
                soundControls[soundName].volume = volume;
                saveSoundControl(soundName, soundControls[soundName]);
                
                // تحديث العرض المرئي
                const volumeFill = e.target.nextElementSibling;
                if (volumeFill && volumeFill.classList.contains('volume-fill')) {
                    volumeFill.style.width = `${volume * 100}%`;
                }
                
                // تحديث النسبة المئوية
                const volumePercent = e.target.parentElement.querySelector('.volume-percent');
                if (volumePercent) {
                    volumePercent.textContent = `${Math.round(volume * 100)}%`;
                }
                
                // تشغيل صوت تجريبي
                if (soundControls[soundName].enabled && audioSystem) {
                    const originalVolume = audioSystem.sounds[soundName]?.volume || 0;
                    audioSystem.sounds[soundName].volume = volume;
                    audioSystem.play(soundName, true);
                    setTimeout(() => {
                        if (audioSystem.sounds[soundName]) {
                            audioSystem.sounds[soundName].volume = originalVolume;
                        }
                    }, 100);
                }
            });
            
            // تحديث العرض الأولي
            const volumeFill = volumeSlider.nextElementSibling;
            if (volumeFill && volumeFill.classList.contains('volume-fill')) {
                volumeFill.style.width = `${(soundControls[soundName].volume || 0.7) * 100}%`;
            }
        };
        
        // زر اختبار الصوت
        const testBtn = controlElement.querySelector('.test-sound-btn');
        if (testBtn) {
            testBtn.addEventListener('click', () => {
                if (soundControls[soundName].enabled && audioSystem) {
                    audioSystem.play(soundName, true);
                }
            });
        };
    });
    
    // زر تفعيل/إلغاء تفعيل جميع الأصوات
    const toggleAllSoundsBtn = document.getElementById('toggle-all-sounds');
    if (toggleAllSoundsBtn) {
        toggleAllSoundsBtn.addEventListener('click', () => {
            const allEnabled = Object.values(soundControls).every(s => s.enabled);
            const newState = !allEnabled;
            
            Object.keys(soundControls).forEach(soundName => {
                soundControls[soundName].enabled = newState;
                saveSoundControl(soundName, soundControls[soundName]);
                
                // تحديث الواجهة
                const controlElement = document.getElementById(`${soundName}-control`);
                if (controlElement) {
                    const toggle = controlElement.querySelector('.sound-toggle');
                    if (toggle) toggle.checked = newState;
                }
            });
            
            if (audioSystem) {
                if (newState) {
                    audioSystem.resumeBackgroundMusic();
                } else {
                    audioSystem.pauseBackgroundMusic();
                }
            }
            
            showToast(newState ? 'تم تفعيل جميع الأصوات' : 'تم إيقاف جميع الأصوات');
        });
    };
    
    // ضبط مستوى الصوت العام
    const masterVolumeSlider = document.getElementById('master-volume');
    if (masterVolumeSlider) {
        masterVolumeSlider.addEventListener('input', (e) => {
            const volume = e.target.value / 100;
            if (audioSystem) {
                audioSystem.setVolume(volume);
            }
            
            // تحديث عرض مستوى الصوت
            const volumeFill = e.target.nextElementSibling;
            if (volumeFill && volumeFill.classList.contains('volume-fill')) {
                volumeFill.style.width = `${volume * 100}%`;
            }
            
            // تحديث النسبة المئوية
            const volumePercent = e.target.parentElement.querySelector('.volume-percent');
            if (volumePercent) {
                volumePercent.textContent = `${Math.round(volume * 100)}%`;
            }
            
            // تشغيل صوت تجريبي
            if (audioSystem) {
                audioSystem.play('click', true);
            }
        });
    };
}

// حفظ الإعدادات في قاعدة البيانات
function saveSetting(settingId, value) {
    if (!db) return;
    
    const transaction = db.transaction(['settings'], 'readwrite');
    const settingsStore = transaction.objectStore('settings');
    settingsStore.put({ id: settingId, value: value });
    
    // حفظ إعدادات الصوت في localStorage أيضًا للوصول السريع
    if (settingId === 'sound') {
        localStorage.setItem('soundEnabled', value);
    } else if (settingId === 'vibration') {
        localStorage.setItem('vibrationEnabled', value);
    } else if (settingId === 'notifications') {
        localStorage.setItem('notificationsEnabled', value);
    }
}

// تغيير اللون
function handleColorChange(colorOption) {
    document.querySelectorAll('.color-option').forEach(opt => {
        opt.classList.remove('active');
        opt.setAttribute('aria-checked', 'false');
    });
    
    colorOption.classList.add('active');
    colorOption.setAttribute('aria-checked', 'true');
    
    const newColor = colorOption.getAttribute('data-color');
    
    // تحديث المتغيرات CSS
    document.documentElement.style.setProperty('--primary', newColor);
    document.documentElement.style.setProperty('--card-back', newColor);
    document.documentElement.style.setProperty('--gradient', `linear-gradient(120deg, ${newColor}, #fd79a8)`);
    
    // حفظ اللون المفضل
    localStorage.setItem('preferredColor', newColor);
}

// استعادة اللون المحفوظ
function restorePreferredColor() {
    const savedColor = localStorage.getItem('preferredColor');
    if (savedColor) {
        const colorOption = document.querySelector(`.color-option[data-color="${savedColor}"]`);
        if (colorOption) {
            handleColorChange(colorOption);
        }
    }
}

// مشاركة التطبيق
function shareApp() {
    if (navigator.share) {
        navigator.share({
            title: 'أبطال البطاقات',
            text: 'جرب هذه اللعبة المثيرة للذكاء!',
            url: window.location.href
        })
        .then(() => console.log('تم المشاركة بنجاح'))
        .catch((error) => {
            console.log('خطأ في المشاركة:', error);
            showToast('ميزة المشاركة غير مدعومة في متصفحك');
        });
    } else {
        showToast('ميزة المشاركة غير مدعومة في متصفحك');
    }
}

// تقييم التطبيق
function rateApp() {
    // يمكن تغيير هذا الرابط ليناسب متجر التطبيقات المستهدف
    const ratingUrl = 'https://play.google.com/store/apps/details?id=com.example.cardheroes';
    
    // عرض خيارات التقييم
    const ratingOptions = [
        { text: '⭐⭐⭐⭐⭐ ممتاز', value: 5 },
        { text: '⭐⭐⭐⭐ جيد جداً', value: 4 },
        { text: '⭐⭐⭐ متوسط', value: 3 },
        { text: '⭐⭐ ضعيف', value: 2 },
        { text: '⭐ سيء', value: 1 }
    ];
    
    const ratingHTML = `
        <div class="cultural-puzzle-overlay active" id="rating-overlay">
            <div class="cultural-puzzle-container glass-effect">
                <div class="puzzle-header">
                    <h2>تقييم التطبيق</h2>
                </div>
                <div class="puzzle-question">كيف تقيم تجربتك مع تطبيق أبطال البطاقات؟</div>
                <div class="puzzle-options">
                    ${ratingOptions.map(option => `
                        <button class="puzzle-option glass-effect-light" data-value="${option.value}">
                            ${option.text}
                        </button>
                    `).join('')}
                </div>
                <div class="puzzle-result" id="rating-result"></div>
                <button class="btn" id="close-rating-btn">إغلاق</button>
            </div>
        </div>
    `;
    
    document.body.insertAdjacentHTML('beforeend', ratingHTML);
    
    document.querySelectorAll('#rating-overlay .puzzle-option').forEach(option => {
        option.addEventListener('click', function() {
            const ratingValue = parseInt(this.dataset.value);
            const resultElement = document.getElementById('rating-result');
            
            if (ratingValue >= 4) {
                resultElement.innerHTML = `<i class="fas fa-smile"></i> شكراً لتقييمك الإيجابي!`;
                resultElement.className = 'puzzle-result correct';
                
                // توجيه المستخدم لكتابة مراجعة في المتجر
                setTimeout(() => {
                    window.open(ratingUrl, '_blank');
                }, 1500);
            } else {
                resultElement.innerHTML = `<i class="fas fa-frown"></i> نأسف لتجربتك. سنعمل على تحسين التطبيق.`;
                resultElement.className = 'puzzle-result incorrect';
                
                // طلب ملاحظات من المستخدم
                showFeedbackForm();
            }
        });
    });
    
    document.getElementById('close-rating-btn').addEventListener('click', () => {
        document.getElementById('rating-overlay').remove();
    });
}

// إعادة ضبط التطبيق
function resetApp() {
    const confirmationMessage = `
        <div class="cultural-puzzle-overlay active" id="reset-confirmation">
            <div class="cultural-puzzle-container glass-effect">
                <div class="puzzle-header">
                    <h2>⚠️ تأكيد إعادة الضبط</h2>
                </div>
                <div class="puzzle-question">
                    هل أنت متأكد من أنك تريد إعادة ضبط التطبيق؟<br>
                    <strong>هذا الإجراء لا يمكن التراجع عنه وسيتم:</strong><br>
                    • حذف جميع الإحصائيات<br>
                    • حذف جميع الإنجازات<br>
                    • إعادة جميع المستويات للحالة الافتراضية<br>
                    • حذف جميع الإعدادات الشخصية
                </div>
                <div class="puzzle-options" style="grid-template-columns: 1fr 1fr; gap: 10px;">
                    <button class="puzzle-option glass-effect-light" style="background: rgba(214, 48, 49, 0.3);" id="confirm-reset">
                        <i class="fas fa-check"></i> نعم، إعادة ضبط
                    </button>
                    <button class="puzzle-option glass-effect-light" style="background: rgba(0, 184, 148, 0.3);" id="cancel-reset">
                        <i class="fas fa-times"></i> إلغاء
                    </button>
                </div>
            </div>
        </div>
    `;
    
    document.body.insertAdjacentHTML('beforeend', confirmationMessage);
    
    document.getElementById('confirm-reset').addEventListener('click', () => {
        performReset();
        document.getElementById('reset-confirmation').remove();
    });
    
    document.getElementById('cancel-reset').addEventListener('click', () => {
        document.getElementById('reset-confirmation').remove();
    });
}

// تنفيذ إعادة الضبط
function performReset() {
    // حذف قاعدة البيانات
    indexedDB.deleteDatabase('CardGameDB');
    
    // حذف localStorage
    localStorage.clear();
    
    // إعادة تحميل الصفحة
    showToast('تم إعادة ضبط التطبيق بنجاح');
    setTimeout(() => {
        location.reload();
    }, 1500);
}

// عرض دليل المساعدة
function showHelpGuide() {
    const helpContent = [
        {
            title: "🎮 كيف تلعب",
            content: "1. اختر مرحلة من الخريطة<br>2. اختر المستوى المناسب<br>3. انقر على بطاقات اللغز لقلبها<br>4. اختر البطاقة الصحيحة للحل<br>5. حاول إيجاد 3 تطابقات صحيحة"
        },
        {
            title: "🏆 النظام النقاطي",
            content: "• إجابة صحيحة: +1 نقطة<br>• إجابة خاطئة: -1 نقطة<br>• حل لغز ثقافي: +1 نقطة<br>• إكمال مستوى: +3 نقاط<br>• التحدي اليومي: نقاط مضاعفة"
        },
        {
            title: "💡 التلميحات والمساعدة",
            content: "• تكلفة التلميح: 10 نقاط<br>• شخصية المرشد تساعدك<br>• الألغاز الثقافية تعطيك نقاطاً إضافية<br>• حاول إكمال المستويات بدون تلميحات لإنجاز خاص"
        },
        {
            title: "⚙️ الإعدادات المتقدمة",
            content: "• يمكنك تغيير الألوان<br>• تفعيل/تعطيل الصوت والاهتزاز<br>• التحكم في مستوى كل صوت على حدة<br>• التحكم في مستوى الصوت العام<br>• تفعيل/تعطيل الإشعارات<br>• تثبيت التطبيق على جهازك"
        },
        {
            title: "🔊 نظام الصوت المتطور",
            content: "• تحكم كامل في كل صوت على حدة<br>• ضبط مستوى الصوت لكل تأثير<br>• اختبار الصوت قبل الحفظ<br>• تفعيل/إلغاء تفعيل جميع الأصوات<br>• حفظ التفضيلات تلقائياً"
        }
    ];
    
    const helpHTML = `
        <div class="cultural-puzzle-overlay active" id="help-overlay">
            <div class="cultural-puzzle-container glass-effect" style="max-width: 600px;">
                <div class="puzzle-header">
                    <h2>📚 دليل المساعدة</h2>
                    <span class="puzzle-category">كل ما تحتاج معرفته عن اللعبة</span>
                </div>
                <div class="help-content" style="text-align: right; max-height: 400px; overflow-y: auto;">
                    ${helpContent.map(section => `
                        <div class="help-section" style="margin-bottom: 20px; padding: 15px; background: rgba(255,255,255,0.05); border-radius: 10px;">
                            <h3 style="color: var(--accent); margin-bottom: 10px;">${section.title}</h3>
                            <p style="line-height: 1.6;">${section.content}</p>
                        </div>
                    `).join('')}
                </div>
                <div style="display: flex; justify-content: center; margin-top: 20px; gap: 10px;">
                    <button class="btn" id="close-help-btn">
                        <i class="fas fa-times"></i> إغلاق
                    </button>
                    <button class="btn" id="mascot-help-btn" style="background: linear-gradient(135deg, #6c5ce7, #a29bfe);">
                        <i class="fas fa-robot"></i> اسأل المساعد
                    </button>
                </div>
            </div>
        </div>
    `;
    
    document.body.insertAdjacentHTML('beforeend', helpHTML);
    
    document.getElementById('close-help-btn').addEventListener('click', () => {
        document.getElementById('help-overlay').remove();
    });
    
    document.getElementById('mascot-help-btn').addEventListener('click', () => {
        document.getElementById('help-overlay').remove();
        if (mascotSystem) {
            mascotSystem.showMessage();
        }
    });
}

// عرض نموذج الاتصال
function showContactForm() {
    const contactHTML = `
        <div class="cultural-puzzle-overlay active" id="contact-overlay">
            <div class="cultural-puzzle-container glass-effect" style="max-width: 500px;">
                <div class="puzzle-header">
                    <h2>📞 اتصل بنا</h2>
                    <span class="puzzle-category">نحن هنا لمساعدتك</span>
                </div>
                <div class="contact-form" style="text-align: right;">
                    <div class="form-group" style="margin-bottom: 15px;">
                        <label for="contact-name" style="display: block; margin-bottom: 5px; font-weight: 600;">
                            <i class="fas fa-user"></i> الاسم
                        </label>
                        <input type="text" id="contact-name" 
                               placeholder="أدخل اسمك" 
                               style="width: 100%; padding: 10px; border-radius: 8px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: white;">
                    </div>
                    <div class="form-group" style="margin-bottom: 15px;">
                        <label for="contact-email" style="display: block; margin-bottom: 5px; font-weight: 600;">
                            <i class="fas fa-envelope"></i> البريد الإلكتروني
                        </label>
                        <input type="email" id="contact-email" 
                               placeholder="أدخل بريدك الإلكتروني" 
                               style="width: 100%; padding: 10px; border-radius: 8px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: white;">
                    </div>
                    <div class="form-group" style="margin-bottom: 15px;">
                        <label for="contact-message" style="display: block; margin-bottom: 5px; font-weight: 600;">
                            <i class="fas fa-comment"></i> الرسالة
                        </label>
                        <textarea id="contact-message" 
                                  rows="4" 
                                  placeholder="أدخل رسالتك هنا..." 
                                  style="width: 100%; padding: 10px; border-radius: 8px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: white; resize: vertical;"></textarea>
                    </div>
                    <div class="form-group" style="margin-bottom: 20px;">
                        <label for="contact-type" style="display: block; margin-bottom: 5px; font-weight: 600;">
                            <i class="fas fa-tag"></i> نوع الرسالة
                        </label>
                        <select id="contact-type" 
                                style="width: 100%; padding: 10px; border-radius: 8px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: white;">
                            <option value="bug">تقرير عن خطأ</option>
                            <option value="suggestion">اقتراح تحسين</option>
                            <option value="question">سؤال</option>
                            <option value="other">أخرى</option>
                        </select>
                    </div>
                </div>
                <div style="display: flex; justify-content: center; gap: 10px;">
                    <button class="btn" id="send-contact-btn">
                        <i class="fas fa-paper-plane"></i> إرسال
                    </button>
                    <button class="btn" id="close-contact-btn" style="background: rgba(214, 48, 49, 0.3);">
                        <i class="fas fa-times"></i> إلغاء
                    </button>
                </div>
                <div class="contact-result" id="contact-result" style="margin-top: 15px; display: none;"></div>
            </div>
        </div>
    `;
    
    document.body.insertAdjacentHTML('beforeend', contactHTML);
    
    // تعبئة البيانات تلقائياً إذا كانت موجودة
    const contactName = document.getElementById('contact-name');
    const contactEmail = document.getElementById('contact-email');
    
    if (userName && contactName) {
        contactName.value = userName;
    }
    if (userEmail && contactEmail) {
        contactEmail.value = userEmail;
    }
    
    document.getElementById('send-contact-btn').addEventListener('click', () => {
        sendContactMessage();
    });
    
    document.getElementById('close-contact-btn').addEventListener('click', () => {
        document.getElementById('contact-overlay').remove();
    });
}

// إرسال رسالة الاتصال
function sendContactMessage() {
    const name = document.getElementById('contact-name').value;
    const email = document.getElementById('contact-email').value;
    const message = document.getElementById('contact-message').value;
    const type = document.getElementById('contact-type').value;
    const resultElement = document.getElementById('contact-result');
    
    if (!name || !email || !message) {
        resultElement.innerHTML = '<div style="color: #fdcb6e; padding: 10px; background: rgba(253, 203, 110, 0.1); border-radius: 8px;">الرجاء ملء جميع الحقول المطلوبة</div>';
        resultElement.style.display = 'block';
        return;
    }
    
    // في بيئة حقيقية، هنا سيكون هناك كود لإرسال البيانات للخادم
    // الآن سنعرض رسالة نجاح وهمية
    
    resultElement.innerHTML = `
        <div style="color: #00b894; padding: 10px; background: rgba(0, 184, 148, 0.1); border-radius: 8px;">
            <i class="fas fa-check-circle"></i> تم إرسال رسالتك بنجاح!<br>
            سنقوم بالرد عليك في أقرب وقت ممكن.
        </div>
    `;
    resultElement.style.display = 'block';
    
    // إخفاء النموذج بعد 3 ثوانٍ
    setTimeout(() => {
        document.getElementById('contact-overlay').remove();
        showToast('شكراً لتواصلك معنا!');
    }, 3000);
}

// عرض سياسة الخصوصية
function showPrivacyPolicy() {
    const privacyHTML = `
        <div class="cultural-puzzle-overlay active" id="privacy-overlay">
            <div class="cultural-puzzle-container glass-effect" style="max-width: 700px; max-height: 80vh;">
                <div class="puzzle-header">
                    <h2>🔒 سياسة الخصوصية</h2>
                    <span class="puzzle-category">آخر تحديث: ${new Date().toLocaleDateString('ar-SA')}</span>
                </div>
                <div class="privacy-content" style="text-align: right; max-height: 400px; overflow-y: auto; line-height: 1.8;">
                    <div class="privacy-section" style="margin-bottom: 20px;">
                        <h3 style="color: var(--accent); margin-bottom: 10px;">1. المعلومات التي نجمعها</h3>
                        <p>نحن نحترم خصوصيتك. المعلومات التي قد نجمعها:</p>
                        <ul style="padding-right: 20px; margin-top: 10px;">
                            <li>الإحصائيات والنتائج داخل اللعبة</li>
                            <li>الإعدادات المفضلة (الألوان، الصوت، الاهتزاز)</li>
                            <li>الإنجازات والمستويات المكتملة</li>
                            <li>وقت اللعب والتفاعلات</li>
                            <li>تفضيلات الصوت لكل صوت على حدة</li>
                        </ul>
                    </div>
                    
                    <div class="privacy-section" style="margin-bottom: 20px;">
                        <h3 style="color: var(--accent); margin-bottom: 10px;">2. كيفية استخدام المعلومات</h3>
                        <p>نستخدم المعلومات ل:</p>
                        <ul style="padding-right: 20px; margin-top: 10px;">
                            <li>تحسين تجربة اللعب</li>
                            <li>تتبع التقدم والإنجازات</li>
                            <li>تخصيص الإعدادات والصوت</li>
                            <li>تحليل أداء التطبيق</li>
                            <li>ضبط تجربة الصوت حسب تفضيلاتك</li>
                        </ul>
                    </div>
                    
                    <div class="privacy-section" style="margin-bottom: 20px;">
                        <h3 style="color: var(--accent); margin-bottom: 10px;">3. تخزين البيانات</h3>
                        <p>جميع البيانات تخزن محلياً على جهازك:</p>
                        <ul style="padding-right: 20px; margin-top: 10px;">
                            <li>لا نرسل أي بيانات إلى خوادم خارجية</li>
                            <li>البيانات محفوظة في IndexedDB و localStorage</li>
                            <li>يمكنك حذف البيانات في أي وقت من إعدادات التطبيق</li>
                            <li>تفضيلات الصوت تحفظ تلقائياً وتطبق فوراً</li>
                        </ul>
                    </div>
                    
                    <div class="privacy-section" style="margin-bottom: 20px;">
                        <h3 style="color: var(--accent); margin-bottom: 10px;">4. حقوقك</h3>
                        <p>لديك الحق في:</p>
                        <ul style="padding-right: 20px; margin-top: 10px;">
                            <li>حذف بياناتك في أي وقت</li>
                            <li>إعادة ضبط التطبيق</li>
                            <li>عدم مشاركة أي معلومات شخصية</li>
                            <li>تعطيل الإشعارات والتتبع</li>
                            <li>التحكم الكامل في كل صوت على حدة</li>
                            <li>ضبط مستوى الصوت العام</li>
                        </ul>
                    </div>
                    
                    <div class="privacy-section">
                        <h3 style="color: var(--accent); margin-bottom: 10px;">5. التغييرات على السياسة</h3>
                        <p>قد نقوم بتحديث سياسة الخصوصية هذه من وقت لآخر. سنعلمك بأي تغييرات جوهرية عن طريق إشعار في التطبيق.</p>
                    </div>
                </div>
                <div style="display: flex; justify-content: center; margin-top: 20px;">
                    <button class="btn" id="close-privacy-btn">
                        <i class="fas fa-check"></i> فهمت وقبلت السياسة
                    </button>
                </div>
            </div>
        </div>
    `;
    
    document.body.insertAdjacentHTML('beforeend', privacyHTML);
    
    document.getElementById('close-privacy-btn').addEventListener('click', () => {
        localStorage.setItem('privacyAccepted', 'true');
        document.getElementById('privacy-overlay').remove();
        showToast('شكراً لقراءة سياسة الخصوصية');
    });
}

// تثبيت التطبيق
function installApp() {
    if (deferredPrompt) {
        deferredPrompt.prompt();
        deferredPrompt.userChoice.then((choiceResult) => {
            if (choiceResult.outcome === 'accepted') {
                console.log('تم قبول التثبيت');
                showToast('جاري تثبيت التطبيق...');
                
                // إخفاء زر التثبيت بعد النجاح
                const installBtn = document.getElementById('install-app');
                if (installBtn) installBtn.style.display = 'none';
            } else {
                console.log('تم رفض التثبيت');
            }
            deferredPrompt = null;
        });
    }
}

// فتح نموذج الملاحظات
function showFeedbackForm() {
    setTimeout(() => {
        const feedbackHTML = `
            <div class="cultural-puzzle-overlay active" id="feedback-overlay">
                <div class="cultural-puzzle-container glass-effect" style="max-width: 500px;">
                    <div class="puzzle-header">
                        <h2>💬 ملاحظاتك تهمنا</h2>
                        <span class="puzzle-category">ساعدنا على التحسين</span>
                    </div>
                    <div class="feedback-form" style="text-align: right;">
                        <div class="form-group" style="margin-bottom: 15px;">
                            <label style="display: block; margin-bottom: 5px; font-weight: 600;">
                                ما الذي يمكننا تحسينه؟
                            </label>
                            <textarea id="feedback-message" 
                                      rows="4" 
                                      placeholder="أخبرنا بتجربتك وما يمكننا تحسينه..." 
                                      style="width: 100%; padding: 10px; border-radius: 8px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: white; resize: vertical;"></textarea>
                        </div>
                    </div>
                    <div style="display: flex; justify-content: center; gap: 10px;">
                        <button class="btn" id="send-feedback-btn">
                            <i class="fas fa-paper-plane"></i> إرسال الملاحظات
                        </button>
                        <button class="btn" id="close-feedback-btn" style="background: rgba(214, 48, 49, 0.3);">
                            <i class="fas fa-times"></i> تخطي
                        </button>
                    </div>
                    <div class="feedback-result" id="feedback-result" style="margin-top: 15px; display: none;"></div>
                </div>
            </div>
        `;
        
        document.body.insertAdjacentHTML('beforeend', feedbackHTML);
        
        document.getElementById('send-feedback-btn').addEventListener('click', () => {
            const feedback = document.getElementById('feedback-message').value;
            if (feedback.trim()) {
                // هنا يمكن إرسال الملاحظات للخادم
                document.getElementById('feedback-result').innerHTML = `
                    <div style="color: #00b894; padding: 10px; background: rgba(0, 184, 148, 0.1); border-radius: 8px;">
                        <i class="fas fa-check-circle"></i> شكراً لك على ملاحظاتك القيمة!
                    </div>
                `;
                document.getElementById('feedback-result').style.display = 'block';
                
                setTimeout(() => {
                    document.getElementById('feedback-overlay').remove();
                }, 2000);
            }
        });
        
        document.getElementById('close-feedback-btn').addEventListener('click', () => {
            document.getElementById('feedback-overlay').remove();
        });
    }, 1000);
}

// التحقق من تثبيت التطبيق
function checkInstallPrompt() {
    window.addEventListener('beforeinstallprompt', (e) => {
        e.preventDefault();
        deferredPrompt = e;
        const installBtn = document.getElementById('install-app');
        if (installBtn) {
            installBtn.style.display = 'inline-flex';
            
            // إضافة تأثير مميز لزر التثبيت
            installBtn.classList.add('pulse');
            setTimeout(() => {
                installBtn.classList.remove('pulse');
            }, 3000);
        }
    });
    
    // التحقق مما إذا كان التطبيق مثبتاً بالفعل
    window.addEventListener('appinstalled', () => {
        const installBtn = document.getElementById('install-app');
        if (installBtn) installBtn.style.display = 'none';
        deferredPrompt = null;
        showToast('تم تثبيت التطبيق بنجاح!');
    });
}

// تهيئة نظام التحكم في الصوت
function initSoundControls() {
    // التحقق من وجود مخزن soundSettings في قاعدة البيانات
    if (!db.objectStoreNames.contains('soundSettings')) {
        // تحديث قاعدة البيانات إذا لزم الأمر
        const request = indexedDB.open('CardGameDB', 11); // زيادة الإصدار
        request.onupgradeneeded = (event) => {
            const db = event.target.result;
            if (!db.objectStoreNames.contains('soundSettings')) {
                db.createObjectStore('soundSettings', { keyPath: 'id' });
            }
        };
    }
    
    // إنشاء عناصر التحكم في الواجهة إذا لم تكن موجودة
    createSoundControlsUI();
}

// إنشاء واجهة التحكم في الأصوات
function createSoundControlsUI() {
    const soundControlsContainer = document.getElementById('sound-controls-container');
    if (!soundControlsContainer) return;
    
    // إنشاء HTML لعناصر التحكم في الصوت
    let soundControlsHTML = `
        <div class="setting-section glass-effect">
            <div class="setting-header">
                <i class="fas fa-sliders-h"></i>
                <h3>التحكم في مستوى الصوت العام</h3>
            </div>
            <div class="sound-control-group">
                <div class="volume-slider-container">
                    <div class="volume-slider">
                        <input type="range" id="master-volume" class="sound-volume" 
                               min="0" max="100" value="${(audioSystem ? audioSystem.volume * 100 : 70)}">
                        <div class="volume-fill"></div>
                    </div>
                    <div class="volume-percent">${Math.round((audioSystem ? audioSystem.volume * 100 : 70))}%</div>
                </div>
                <button class="test-sound-btn" onclick="if(audioSystem) audioSystem.play('click', true)">
                    <i class="fas fa-play"></i> اختبار
                </button>
            </div>
        </div>
        
        <div class="setting-section glass-effect">
            <div class="setting-header">
                <i class="fas fa-music"></i>
                <h3>التحكم في كل صوت على حدة</h3>
                <button class="btn-sm" id="toggle-all-sounds">
                    <i class="fas fa-bolt"></i> تفعيل/إلغاء الكل
                </button>
            </div>
    `;
    
    // إنشاء عناصر تحكم لكل صوت
    const soundDescriptions = {
        click: { icon: 'fa-mouse-pointer', label: 'صوت النقر' },
        flip: { icon: 'fa-sync-alt', label: 'صوت قلب البطاقة' },
        success: { icon: 'fa-check-circle', label: 'صوت النجاح' },
        error: { icon: 'fa-times-circle', label: 'صوت الخطأ' },
        win: { icon: 'fa-trophy', label: 'صوت الفوز' },
        notification: { icon: 'fa-bell', label: 'صوت الإشعار' },
        levelComplete: { icon: 'fa-flag-checkered', label: 'صوت إكمال المستوى' },
        star: { icon: 'fa-star', label: 'صوت النجم' },
        background: { icon: 'fa-headphones', label: 'موسيقى الخلفية' },
        culturalPuzzle: { icon: 'fa-brain', label: 'صوت اللغز الثقافي' },
        hint: { icon: 'fa-lightbulb', label: 'صوت التلميح' },
        unlock: { icon: 'fa-unlock', label: 'صوت فتح المستوى' },
        coin: { icon: 'fa-coins', label: 'صوت النقاط' },
        achievement: { icon: 'fa-medal', label: 'صوت الإنجاز' },
        timer: { icon: 'fa-hourglass-half', label: 'صوت المؤقت' },
        applause: { icon: 'fa-hands-clapping', label: 'صوت التصفيق' }
    };
    
    Object.keys(soundControls).forEach(soundName => {
        const desc = soundDescriptions[soundName] || { icon: 'fa-volume-up', label: soundName };
        const settings = soundControls[soundName] || { enabled: true, volume: 0.7 };
        
        soundControlsHTML += `
            <div class="sound-control-item" id="${soundName}-control">
                <div class="sound-control-info">
                    <i class="fas ${desc.icon}"></i>
                    <div>
                        <div class="sound-label">${desc.label}</div>
                        <div class="sound-status">${settings.enabled ? 'مفعل' : 'معطل'} | ${Math.round(settings.volume * 100)}%</div>
                    </div>
                </div>
                <div class="sound-control-actions">
                    <label class="switch">
                        <input type="checkbox" class="sound-toggle" ${settings.enabled ? 'checked' : ''}>
                        <span class="slider"></span>
                    </label>
                    <div class="volume-slider-container">
                        <div class="volume-slider">
                            <input type="range" class="sound-volume" 
                                   min="0" max="100" value="${settings.volume * 100}">
                            <div class="volume-fill"></div>
                        </div>
                        <div class="volume-percent">${Math.round(settings.volume * 100)}%</div>
                    </div>
                    <button class="test-sound-btn">
                        <i class="fas fa-play"></i>
                    </button>
                </div>
            </div>
        `;
    });
    
    soundControlsHTML += `
        </div>
    `;
    
    soundControlsContainer.innerHTML = soundControlsHTML;
}

// تهيئة شاشة الإعدادات
function initSettingsScreen() {
    loadUserSettings();
    setupSettingsEventListeners();
    restorePreferredColor();
    checkInstallPrompt();
    initSoundControls();
    
    // التحقق من قبول سياسة الخصوصية
    if (!localStorage.getItem('privacyAccepted')) {
        setTimeout(() => {
            showPrivacyPolicy();
        }, 3000);
    }
}

// إضافة التحكم في الصوت إلى نظام الصوت
function enhanceAudioSystem() {
    if (!audioSystem) return;
    
    // نسخ الدالة الأصلية
    const originalPlay = audioSystem.play;
    
    // تعديل دالة play للتحقق من إعدادات الصوت
    audioSystem.play = function(soundName, force = false) {
        // التحقق من إعداد الصوت العام
        if (!this.isEnabled && !force) return;
        
        // التحقق من إعداد الصوت المحدد
        const soundControl = soundControls[soundName];
        if (soundControl && !soundControl.enabled && !force) return;
        
        // ضبط مستوى الصوت
        const sound = this.sounds[soundName];
        if (sound && soundControl) {
            sound.volume = soundControl.volume;
        }
        
        // استدعاء الدالة الأصلية
        return originalPlay.call(this, soundName, force);
    };
    
    // تعديل دالة setVolume لضبط مستوى الصوت العام
    const originalSetVolume = audioSystem.setVolume;
    audioSystem.setVolume = function(volume) {
        this.volume = Math.max(0, Math.min(1, volume));
        
        // ضبط مستوى الصوت لكل صوت مع مراعاة الإعدادات الفردية
        Object.keys(this.sounds).forEach(soundName => {
            const sound = this.sounds[soundName];
            const soundControl = soundControls[soundName];
            
            if (sound && soundControl) {
                // تطبيق مستوى الصوت الفردي مضروباً في مستوى الصوت العام
                sound.volume = soundControl.volume * this.volume;
            } else if (sound) {
                sound.volume = this.volume;
            }
        });
        
        // ضبط موسيقى الخلفية بشكل خاص
        this.sounds.background.volume = this.volume * 0.3;
        
        // حفظ الإعداد
        if (typeof saveSetting === 'function') {
            saveSetting('volume', this.volume);
        }
    };
}

// تهيئة النظام عند تحميل الصفحة
if (typeof audioSystem !== 'undefined') {
    enhanceAudioSystem();
}

// التصدير للاستخدام في الملفات الأخرى
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        initSettingsScreen,
        soundControls,
        vibrationEnabled,
        soundEnabled,
        notificationsEnabled
    };
}
