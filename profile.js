...
function showAchievementDetails(achievement, isEarned, earnedDate) {
        ...
        <div class="achievement-details-overlay" id="achievement-details">
            <div class="achievement-details-container glass-effect">
                ...
                <div class="details-actions" style="display: flex; gap: 10px;">
                    <button class="btn ripple close-details-btn" style="flex: 1; background: rgba(255,255,255,0.1);">
                        إغلاق
                    </button>
                    ${isEarned ? `
                    <button class="btn ripple share-achievement-btn" style="flex: 1; background: var(--accent);">
    ...
    // إضافة مستمعي الأحداث للنافذة الجديدة
    const closeBtn = document.querySelector('#achievement-details .close-details-btn');
    if (closeBtn) {
    ...
    const shareBtn = document.querySelector('#achievement-details .share-achievement-btn');
    if (shareBtn) {
    ...
    // إلغاء أي مستمع حدث للنقر خارج النافذة
    const overlay = document.getElementById('achievement-details');
    if (overlay) {
        // فقط إغلاق عند النقر على زر الإغلاق - لا إغلاق تلقائي
        overlay.style.pointerEvents = 'auto';
        
        // إغلاق عند الضغط على زر Escape فقط
        ...
        setTimeout(() => {
            ...
            const container = overlay.querySelector('.achievement-details-container');
            gsap.from(container, {
                scale: 0.8,
                opacity: 0,
                duration: 0.4,
...
// إغلاق نافذة تفاصيل الإنجاز (مصحح)
...
function initProfileStyles() {
    ...
    style.textContent = `
        ...
        /* إلغاء أي تأثيرات خارجية قد تسبب الإغلاق */
        .achievement-details-overlay * {
            pointer-events: auto;
        }
    `;
