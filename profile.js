...
function showAchievementDetails(achievement, isEarned, earnedDate) {
    ...
    if (overlay) {
        ...
        startAutoCloseCountdown(achievement.title, isEarned);
...
// بدء العد التنازلي للإغلاق التلقائي
function startAutoCloseCountdown(achievementTitle, isEarned) {
    let secondsLeft = 4;
    ...
    const countdownInterval = setInterval(() => {
        ...
        if (secondsLeft <= 0) {
            ...
            // إذا كان الإنجاز مكتسباً، عرض رسالة قصيرة
            if (isEarned) {
                setTimeout(() => {
                    showToast(`تم عرض تفاصيل إنجاز: ${achievementTitle}`);
                }, 300);
            }
        }
    }, 1000);
...
// مشاركة الإنجاز (لم تعد تستخدم في النافذة المنبثقة)
