...
function loadUserProfileFromDB() {
    ...
    if (!window.db) {
        console.warn('قاعدة البيانات غير متاحة، استخدام البيانات المحلية');
    ...
    try {
        ...
        request.onsuccess = (event) => {
            ...
            if (userData) {
                console.log('تم تحميل بيانات المستخدم من قاعدة البيانات:', userData);
...
// تحميل بيانات المستخدم من البيانات المحلية
function loadUserProfileFromLocal() {
    console.log('جاري تحميل بيانات المستخدم من البيانات المحلية...');
...
function createDefaultUserData() {
    ...
    // تحديث المتغيرات العامة
    window.userName = defaultUserData.name;
    window.userEmail = defaultUserData.email;
    window.totalPoints = defaultUserData.totalPoints;
...
function setupProfileEventListeners() {
    ...
    if (saveProfileBtn) {
        saveProfileBtn.addEventListener('click', saveProfileToDB);
        console.log('تم إعداد مستمع لحفظ الملف الشخصي في قاعدة البيانات');
    } else {
        ...
        setTimeout(() => {
            ...
            if (retryBtn) {
                retryBtn.addEventListener('click', saveProfileToDB);
...
function saveProfileToDB() {
    console.log('جاري حفظ بيانات الملف الشخصي في قاعدة البيانات...');
    ...
    // تحديث بيانات الملف الشخصي المحلية
    ...
    // حفظ في قاعدة البيانات
    if (!saveUserProfileToDB(newName, newEmail)) {
        // إذا فشل الحفظ في قاعدة البيانات، اعرض رسالة واضحة
        showToast('تم حفظ التغييرات مؤقتاً في الذاكرة المحلية');
    }
}

// حفظ بيانات المستخدم في قاعدة البيانات
function saveUserProfileToDB(name, email) {
    if (!window.db) {
    ...
    try {
        ...
        const getRequest = userStore.get('profile');
        ...
        getRequest.onsuccess = () => {
            ...
            const putRequest = userStore.put(userData);
            
            putRequest.onsuccess = () => {
                console.log('تم حفظ بيانات المستخدم في قاعدة البيانات');
                showToast('تم حفظ التغييرات بنجاح');
                
                // تشغيل صوت النجاح إذا كان الصوت مفعلاً
            ...
            putRequest.onerror = (error) => {
                console.error('خطأ في حفظ بيانات المستخدم في قاعدة البيانات:', error);
        ...
        getRequest.onerror = (error) => {
            ...
            showToast('حدث خطأ في الوصول إلى البيانات');
            return false;
        };
...
function updateProfileStats() {
    ...
    // استخدام البيانات المتاحة مباشرة
...
function updateProfileStatsDirect() {
    ...
    if (window.levelsData) {
        // احسب المستويات المكتملة من النقاط (كل مستوى يعطي 3 نقاط كحد أقصى)
        ...
        if (completedLevels > 0) {
            ...
            successRate = Math.round((totalPoints / maxPossiblePoints) * 100);
        }
        
        // تحديث البيانات
        userProfileData.completedLevels = completedLevels;
        userProfileData.successRate = successRate;
        userProfileData.playTime = window.playTime || 0;
...
function updateProfileStatsFromDB() {
    ...
    try {
        // حساب المستويات المكتملة
        calculateCompletedLevels((completedLevels, totalEarnedPoints) => {
            // الحصول على إجمالي النقاط من الإحصائيات
            getTotalPointsFromStats((totalPointsFromStats) => {
                // الحصول على وقت اللعب
                getPlayTimeFromUserData((playTime) => {
                    // حساب معدل النجاح
                    let successRate = 0;
                    if (completedLevels > 0) {
                        const maxPossiblePoints = completedLevels * 3;
                    ...
                    userProfileData.completedLevels = completedLevels;
                    userProfileData.successRate = successRate;
                    userProfileData.playTime = playTime || 0;
                    ...
                    // تحديث المتغيرات العامة
                    window.totalPoints = totalPointsFromStats;
                    window.playTime = playTime || 0;
...
// حساب المستويات المكتملة
function calculateCompletedLevels(callback) {
    const transaction = window.db.transaction(['levels'], 'readonly');
    ...
    countRequest.onsuccess = (event) => {
        ...
        if (cursor) {
            ...
            callback(completedLevels, totalEarnedPoints);
        }
    };
...
// الحصول على إجمالي النقاط من الإحصائيات
function getTotalPointsFromStats(callback) {
    const transaction = window.db.transaction(['stats'], 'readonly');
    const statsStore = transaction.objectStore('stats');
    const request = statsStore.get('totalPoints');
    
    request.onsuccess = (event) => {
        const statsResult = event.target.result;
        const totalPoints = statsResult ? statsResult.value : 0;
        callback(totalPoints);
    };
    
    request.onerror = (error) => {
        console.error('خطأ في الحصول على النقاط الإجمالية:', error);
        callback(0);
    };
}

// الحصول على وقت اللعب من بيانات المستخدم
function getPlayTimeFromUserData(callback) {
    const transaction = window.db.transaction(['user'], 'readonly');
    const userStore = transaction.objectStore('user');
    const request = userStore.get('profile');
    ...
    request.onsuccess = (event) => {
        ...
        const playTime = userResult ? userResult.playTime : 0;
        callback(playTime);
    };
    
    request.onerror = (error) => {
        console.error('خطأ في الحصول على وقت اللعب:', error);
        callback(0);
    };
}
...
function updateProfileStatsUI(completedLevels, totalPoints, successRate, totalLevels) {
    ...
    if (completedLevelsElement) {
        ...
        console.warn('عنصر المستويات المكتملة غير موجود');
    }
    ...
    if (successRateElement) {
        ...
        console.log('تعيين معدل النجاح:', successRate);
    } else {
        console.warn('عنصر معدل النجاح غير موجود');
    }
    ...
    if (playTimeElement) {
        ...
        console.log('تعيين وقت اللعب:', `${hours} س ${minutes} د`);
    } else {
        console.warn('عنصر وقت اللعب غير موجود');
    }
    ...
    if (profileLevelElement) {
        ...
        console.log('تعيين مستوى المستخدم:', userLevel);
    } else {
        console.warn('عنصر مستوى المستخدم غير موجود');
    }
